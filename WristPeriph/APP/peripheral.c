...
// PPG ????????????ms????20ms ?? 50Hz ???????? MAX30102 FIFO ??
...
// GAP GATT Attributes
static uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "Wrist";
...
void Peripheral_Init()
{
    ...
    // Setup the SimpleProfile Characteristic Values
    {
        ...
        uint8_t charValue4[SIMPLEPROFILE_CHAR4_LEN] = {0};   /* ????????????????? Peripheral_WristNotifyJson */
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    if(peripheralConnList.connHandle != GAP_CONNHANDLE_INIT)
    {
        ...
        PRINT("Wrist: reject 2nd link h=%02X (already h=%02X)\n",
...
static void performPeriodicTask(void)
{
    /* Char4 ????????????? Peripheral_WristNotifyJson(hr, spo2) ?? PPG ??????????? */
}

/*********************************************************************
 * @fn      Peripheral_WristNotifyJson
 *
 * @brief   ???? HR/SpO2 ??? JSON ??? Char4 Notify ????? host (ESP32-C3)
 *          JSON: {"id":N,"ts":T,"b_idx":I,"beat_ts":B,"hr":H,"spo2":S}
 *          beat_ts = b_idx * 20 ms??????????????????????????????????
 */
void Peripheral_WristNotifyJson(uint8_t hr, uint8_t spo2, uint16_t beat_idx, uint32_t beat_ts,
                                const ppg_wf_compact_t *wf)
{
    ...
    {
        PRINT("Wrist: JSON encode fail n=%d\n", n);
        return;
    }
    peripheralChar4Notify((uint8_t *)buf, (uint16_t)n);
}
...
static void peripheralChar4Notify(uint8_t *pValue, uint16_t len)
{
    ...
    {
        PRINT("Too large noti len=%u mtu=%u\n", (unsigned)len, (unsigned)peripheralMTU);
        return;
    }
    noti.len = len;
