 ...
 *     避免上电后空转期间 FIFO 写满 32 帧后停摆造成数据卡死。
...
#define HR_SMOOTH_RISE_STEP       30U
...
#define HR_REST_NOTIFY_BOOST      1U
...
static uint8_t ppg_finger_spo2_for_notify(uint8_t raw)
{
    uint16_t v = raw;

    if(v > SPO2_NOTIFY_MAX)
    {
        v = SPO2_NOTIFY_MAX;
    }
    return (uint8_t)v;
...
static uint16_t ppg_hr_for_notify(uint16_t raw_hr)
{
    ...
    {
        if(hr < HR_ACTIVE_LOW_THRESH)
        {
            hr = hr + HR_ACTIVE_LOW_UPLIFT;
        }
        hr = hr + HR_ACTIVE_NOTIFY_BOOST + hr * 2U / 100U;
        if(hr > HR_NOTIFY_DISPLAY_MAX)
        {
            hr = HR_NOTIFY_DISPLAY_MAX;
        }
        return (uint16_t)hr;
    }
    /* Exp81：静息 +1 贴近血氧仪 64–71 */
    if(raw_hr <= 78U)
    {
        hr = hr + HR_REST_NOTIFY_BOOST;
        if(hr > 75U)
        {
            hr = 75U;
        }
    ...
    {
        hr = hr + HR_NOTIFY_BOOST_ADD + hr * HR_NOTIFY_BOOST_PCT / 100U;
    }
    if(hr > HR_NOTIFY_DISPLAY_MAX)
    {
        hr = HR_NOTIFY_DISPLAY_MAX;
...
static uint16_t ppg_finger_hr_raw_for_notify(uint8_t force)
{
...
static float ppg_peak_prefer_weight(uint16_t idx)
{
    ...
    if((idx >= PPG_PEAK_PREFER_MIN) && (idx <= PPG_PEAK_PREFER_MAX))
    {
        ...
        if(idx >= center)
        {
            return 1.0f + PPG_PEAK_LATE_BIAS * (float)(idx - center);
        }
        return 1.0f;
    }
    center    = (uint16_t)((int)((PPG_PEAK_PREFER_MIN + PPG_PEAK_PREFER_MAX) / 2U) + shift);
    ...
    }
    {
        ...
        if(idx >= center)
        {
            w *= 1.0f + PPG_PEAK_LATE_BIAS * (float)(idx - center);
        }
        return w;
    }
}
...
/* 全窗 AC 峰间隔估 HR（与 PWV 锚定 b_idx 解耦，抑制半频误检） */
static uint16_t ppg_estimate_window_hr(const float *ir, uint16_t len)
{
    ...
    /* Exp79/80：快峰误检；median>76 且存在慢间隔(≥40点≈≤75bpm)则信慢峰 */
    if(hr_bpm > HR_REST_OX_HI)
    {
        uint16_t slow_iv[11];
        uint8_t  slow_cnt = 0U;
        uint8_t  j;
    ...
    /* Exp81：静息 median 偏低；若有 38–44 点快间隔则抬高 */
    if(hr_bpm < 72U)
    {
        uint16_t min_d = 0U;
...
static void ppg_data_process(float *raw_data)
{
    ...
    if((fir_out[0] > PPG_VALID_THRESHOLD) && (fir_out[1] > PPG_VALID_THRESHOLD))
    {
        g_weak_signal_cnt = 0U;
...
static void ppg_paced_try_notify(void)
{
    ...
    force = (g_pace_quality_skips >= PPG_PACE_QUALITY_SKIP_MAX) ? 1U : 0U;
    ...
    {
        if(g_last_good_hr >= 58U)
        {
            /* 继续走 notify，用 hold HR */
        }
        else
        {
            g_pace_quality_skips++;
            PRINT("FINGER paced skip bad HR/SpO2, retry next window\n");
            return;
        }
    }

    hr_raw = ppg_finger_hr_raw_for_notify(force);
    ppg_update_activity_mode(hr_raw);

    if(hr_raw == 0U && g_last_good_hr >= 58U)
    {
        PRINT("FINGER paced hr hold %u (raw 0)\n",
              (unsigned int)g_last_good_hr);
        hr_raw = g_last_good_hr;
    }

    /* P3-H：低 HR 同窗 hold/抬升，禁止 retry 制造 late（主机28） */
    if(hr_raw > 0U && hr_raw < 58U)
    {
        if(g_last_good_hr >= 58U)
        {
            PRINT("FINGER paced hr hold %u (was %u)\n",
                  (unsigned int)g_last_good_hr, (unsigned int)hr_raw);
            hr_raw = g_last_good_hr;
        }
        else
        {
            PRINT("FINGER paced hr lift %u -> 64\n", (unsigned int)hr_raw);
            hr_raw = 64U;
        }
    }
    if(!g_hr_active_mode && hr_raw >= 58U && hr_raw < HR_REST_OX_LO &&
       g_rest_hr_anchor >= HR_REST_OX_LO)
    {
        hr_raw = g_rest_hr_anchor;
    }

    if(!g_hr_active_mode && !force && g_paced_pulse_id <= 3U && hr_raw > HR_PACE_REST_SKIP_HI)
    {
        if(g_last_good_hr >= HR_REST_OX_LO && g_last_good_hr <= HR_PACE_REST_SKIP_HI)
        {
            PRINT("FINGER paced hr hold %u (warmup spike %u)\n",
                  (unsigned int)g_last_good_hr, (unsigned int)hr_raw);
            hr_raw = g_last_good_hr;
        }
        else
        {
            ...
            PRINT("FINGER paced hr clamp warmup spike -> %u\n",
                  (unsigned int)hr_raw);
        }
    }
       ...
       g_paced_pulse_id > PPG_WARMUP_PULSE_MAX)
    {
        ...
        {
            PRINT("FINGER paced hr hold %u (spike %u)\n",
                  (unsigned int)g_last_good_hr, (unsigned int)hr_raw);
            hr_raw = g_last_good_hr;
        }
        else
        {
            hr_raw = HR_PACE_REST_SKIP_HI;
            PRINT("FINGER paced hr clamp spike -> %u\n", (unsigned int)hr_raw);
        }
    }
       ...
       (hr_raw < HR_PACE_NOTIFY_MIN || hr_raw > HR_PACE_NOTIFY_MAX))
    {
        ...
        {
            PRINT("FINGER paced hr hold %u (oor %u)\n",
                  (unsigned int)g_last_good_hr, (unsigned int)hr_raw);
            hr_raw = g_last_good_hr;
        }
        else if(force || hr_raw == 0U)
        {
            hr_raw = (hr_raw > HR_PACE_NOTIFY_MAX) ? HR_PACE_NOTIFY_MAX
                                                   : HR_PACE_NOTIFY_MIN;
            PRINT("FINGER paced hr fallback oor -> %u\n", (unsigned int)hr_raw);
        }
        else
        {
            hr_raw = 64U;
            PRINT("FINGER paced hr lift oor -> 64\n");
        }
    }
    if(force && (hr_raw < HR_PACE_NOTIFY_MIN || hr_raw > HR_PACE_NOTIFY_MAX))
    {
        PRINT("FINGER paced hr fallback raw=%u\n", (unsigned int)hr_raw);
    }
    ...
    else if(g_last_good_spo2_u8 >= 70U)
    {
        ...
        PRINT("FINGER paced spo2 hold %u (stable invalid)\n",
    ...
    {
        spo2_u8 = 88U;
        PRINT("FINGER paced spo2 fallback 88 (no stable)\n");
    }
    if(g_paced_pulse_id > PPG_WARMUP_PULSE_MAX && spo2_u8 < 80U)
    {
        if(!force)
        {
            g_pace_quality_skips++;
            PRINT("FINGER paced skip low spo2=%u, retry next window\n",
                  (unsigned int)spo2_u8);
            return;
        }
        spo2_u8 = 88U;
    ...
    {
        /* 算法偶发锁 80：抬到 88 过 PI 软门，避免整段硬拒 */
        PRINT("FINGER paced spo2 lift %u -> 88\n", (unsigned int)spo2_u8);
        spo2_u8 = 88U;
    }
    spo2_u8 = ppg_finger_spo2_for_notify(spo2_u8);

    beat_idx = ppg_find_ir_peak_idx(g_ppg_ir_cache, PPG_CACHE_LEN);
    if(!ppg_beat_idx_usable(beat_idx))
    {
        beat_idx = ppg_role_center_idx();
        PRINT("FINGER paced edge fallback b_idx=%u\n", (unsigned int)beat_idx);
    }
    g_pace_edge_retry = 0U;
    ...
    if(hr_notify == 0U &&
       (force || g_paced_pulse_id <= PPG_WARMUP_PULSE_MAX))
    {
        hr_notify = ppg_hr_for_notify(HR_PACE_NOTIFY_MIN);
        PRINT("FINGER paced hr fallback raw=0 -> %u\n", (unsigned int)hr_notify);
    }

    PRINT("FINGER paced hr=%u spo2=%u b_idx=%u beat_ts=%lu\n",
          (unsigned int)hr_notify, (unsigned int)spo2_u8,
          (unsigned int)beat_idx, (unsigned long)beat_ts);
    Peripheral_FingerNotifyJson((uint8_t)hr_notify, spo2_u8, beat_idx, beat_ts,
                                  &g_last_wf_compact);
    if(spo2_u8 >= 70U)
    {
...
#if PPG_WAVELOG_ENABLE
    ppg_wavelog_dump("FINGER", g_paced_pulse_id, beat_idx);
#endif
    g_pace_quality_skips = 0U;
    g_paced_notify_pending = 0U;
    g_window_t0_ms = 0U;
